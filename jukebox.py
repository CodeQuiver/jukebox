...
def parse_args(raw_args):
    ...
    parser.add_argument('--sound', dest='chosen_sound',
                        help='Name of a specific sound file to play, instead of randomly selecting on in the folder.')
    parser.add_argument('--folder', dest='sound_folder', default=default_sound_folder,
                        help='Path to a specific folder to search instead of the stored setting.')
    parser.add_argument('--set-default-sound-folder', dest='set_default_sound_folder',
                        help='Path to a specific folder to store as the new default.')
    parser.add_argument('--get-default-sound-folder', action='store_true', dest='get_default_sound_folder',
...
def play_random_sound(folder):
    mp3_and_wav_files = [ file for file in os.listdir(folder) if (file.endswith('.mp3') or (file.endswith('.wav')) )]
        ...
        if args.set_default_sound_folder:
            try:
               update_default_folder(args.set_default_sound_folder)
                ...
                print (f"Failed to update default sound folder to '{args.set_default_sound_folder}'; Error: '{e}'")
                raise
            print (f"Successfully updated default sound folder to '{args.set_default_sound_folder}'")
